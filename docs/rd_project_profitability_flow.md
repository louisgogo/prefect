...
技术服务收入和技术服务成本使用不同组织判断，这是从当前 Power BI 度量值直接还原的业务规则，后续若调整必须同步修改代码、测试和本文档。
