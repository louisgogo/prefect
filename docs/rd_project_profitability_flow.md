...
本文档说明 `rd_project_profitability_flow` 的业务口径、数据来源、产品映射、逐步计算公式、收入成本备查、Excel 输出和前端调用方式，供财务、研发管理、数据开发和系统维护人员后续查询。
...
剩余收益_i = 研发相关毛利_i - 费用总额_i
```

流程只输出这一套正常计算口径，不再保留 Power BI 兼容毛利率、剩余收益或口径差额。
...
电子支付收入和成本用于计算销售费用的收入占比及管理费用的成本占比，不作为第二套收益口径对外输出：
...
研发相关收入和研发相关成本使用相同的组织范围：电子支付和技术服务都只保留国际业务中心。
...
三个费用基数都只保留国际业务。费用基数在 Excel 每一产品行重复展示，是为了方便核对分摊池；它们不是每个产品各自拥有的费用。
...
分母包含 `未映射产品` 的研发工时，确保所有已归集工时都参与分摊。
...
收入占比不使用“研发相关收入”，而是沿用已核准的电子支付收入分摊基础。
...
按已核准的分摊池规则，领料和模具合计包含 `未映射产品`，OA扣减合计不包含 `未映射产品`。该规则用于保持已验证的费用分摊金额不变。
...
这是流程唯一对外输出的收益口径，收入、成本、毛利和最终收益均使用一致的国际业务中心范围。
...
流程把实际用于研发相关收入和成本计算的来源行写入 `收入成本备查` 子表。该子表只包含：

- 收入大类为电子支付或技术服务；
- 二级组织为国际业务中心。

备查表保留物料编码、产品映射、收入大类、组织、收入、成本和毛利。其收入、成本合计必须分别等于主表的研发相关收入、研发相关成本合计。
...
| 本期研发相关成本 | 二级组织为国际业务中心的电子支付和技术服务成本 |
...
包含期间、生成时间、项目行数、收入、成本、毛利、费用、剩余收益、备查表行数及合计、产品主数据未承接 OA。
...
逐行列出国际业务中心的电子支付和技术服务收入成本数据，包括期间、产品映射、物料编码、收入大类、一级组织、二级组织、收入、成本和毛利。该表用于追溯主表研发相关收入及成本。
...
```json
{
  ...
  "report": {
    ...
    "size_bytes": 34833,
    "row_count": 176,
    "backup_row_count": 1149,
    "sheet_names": ["研发项目收益", "汇总与校验", "计算口径", "收入成本备查"]
  },
  "validation": {
    "remaining_profit_total": 146749403.63,
    "revenue_cost_backup_revenue_total": 736636078.99,
    "revenue_cost_backup_cost_total": 474249880.54
  }
}
```
...
自2026年7月28日起，研发相关收入和成本均改为只统计二级组织为国际业务中心的电子支付和技术服务；研发、销售和管理费用基数也改为只保留国际业务。用户最初提供的 Excel 使用调整前口径，因此收入、成本、毛利、三项分摊费用、费用总额和剩余收益均不再与原 Excel 一致。直接费用、分摊比例和计算方法未变。
...
| 国际业务中心电子支付成本 | 474,249,880.54 |
| 国际业务中心技术服务成本 | 0.00 |
| 研发相关成本 | 474,249,880.54 |
| 研发相关毛利 | 262,386,198.45 |
| 国际业务研发费用基数 | 63,766,442.65 |
| 国际业务销售费用基数 | 26,289,130.86 |
| 国际业务管理费用基数 | 25,301,321.31 |
| 费用明细汇总 | 115,636,794.82 |
| 剩余收益 | 146,749,403.63 |
| 产品主数据未承接OA费用 | 3,578,465.82 |

新口径相比最初提供的 Excel 减少研发相关收入 `15,470,435.61` 元，减少研发相关成本 `7,129,832.35` 元，同时因三个费用基数只保留国际业务，费用明细汇总减少 `14,244,199.50` 元。综合影响后，剩余收益比最初口径增加 `5,903,596.24` 元。公式、分摊和文件生成校验均已通过。
...
- Excel必须包含4个Sheet且主表行数不为0。
- 三种占比合计应为1或在无数据时为0。
- 三类分摊残差应在容差内。
- `收入成本备查` 的收入、成本合计必须与主表研发相关收入、成本一致。
- 检查 `未映射产品` 金额是否异常增大。
...
| 备查表收入或成本与主表不一致 | 过滤条件或产品映射逻辑发生变化 | 查看校验错误并核对国际业务中心、电子支付和技术服务条件 |
