...
def load_validation_summary(cur, month: str) -> ValidationSummary:
    cur.execute(
        ...
        SELECT
            ...
            COUNT(*) FILTER (
                WHERE NULLIF(BTRIM(staging."来源层级"), '') IS NULL
                  AND fact_source.source_no IS NULL
            ) AS missing_fact_source,
            COUNT(*) FILTER (
                WHERE NULLIF(BTRIM(staging."来源层级"), '') IS NULL
                  AND fact_source.level_count <> 1
            ) AS ambiguous_fact_source,
            COUNT(*) FILTER (
                WHERE staging."数据来源" IS DISTINCT FROM '费用'
            ) AS data_source_mismatch,
            COALESCE(SUM(staging."费用金额"), 0) AS amount_total,
            md5(COALESCE(string_agg(
                md5((to_jsonb(staging) - '来源层级' - '数据来源')::text),
                '' ORDER BY staging.id
...
def apply_backfill(cur, month: str) -> int:
    cur.execute(
        ...
        SET
            "来源层级" = COALESCE(
                NULLIF(BTRIM(staging."来源层级"), ''),
                fact_source.source_level
            ),
            "数据来源" = '费用'
        FROM fact_source
        WHERE staging."来源编号" = fact_source.source_no
          ...
          AND (
              NULLIF(BTRIM(staging."来源层级"), '') IS NULL
              OR staging."数据来源" IS DISTINCT FROM '费用'
          )
        """,
        (f"{month}%", f"{month}%"),
    )
...
def main() -> None:
    ...
    try:
        ...
        print(
            ...
            f"待规范数据来源 {before.data_source_mismatch} 行，"
        ...
        if after.data_source_mismatch:
            raise ValueError(f"回填后仍有 {after.data_source_mismatch} 条数据来源不是“费用”。")
        if after.staging_rows != before.staging_rows:
            raise ValueError("回填前后费用 Staging 行数发生变化，事务已回滚。")
        if after.amount_total != before.amount_total:
            raise ValueError("回填前后费用金额合计发生变化，事务已回滚。")
        if after.protected_checksum != before.protected_checksum:
            raise ValueError("回填修改了来源字段以外的数据，事务已回滚。")
        ...
        if args.apply:
            ...
            conn.rollback()
            print(f"Dry-run通过：将更新 {updated} 行；事务已回滚。使用 --apply 才会提交。")
    except Exception:
        conn.rollback()
        raise
    finally:
