...
def process_manual_revenue_task(
    df_revenue: pd.DataFrame, df_upload_merge_all: pd.DataFrame
) -> pd.DataFrame:
    """
    处理手工分拆的收入数据
...
def process_auto_revenue_task(
    df_revenue: pd.DataFrame, df_upload_merge_all: pd.DataFrame, df_org: pd.DataFrame
) -> pd.DataFrame:
    """
    处理自动归属的收入数据
...
def merge_revenue_data_task(
    df_revenue_bus_hand: pd.DataFrame, df_revenue_bus_auto: pd.DataFrame
) -> pd.DataFrame:
    """
    合并收入数据
...
def pivot_revenue_data_task(df_revenue_bus_all: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        # 替换科目名称
        df_revenue_bus_all_to_profit["prim_subj"] = df_revenue_bus_all_to_profit[
            "prim_subj"
        ].replace(
            {
...
def apply_shared_rate_to_revenue_task(
    df_revenue_bus_all_to_profit: pd.DataFrame, date_range: pd.DatetimeIndex
) -> pd.DataFrame:
    """
...
def save_revenue_detail_task(df: pd.DataFrame, date_range: pd.DatetimeIndex) -> None:
    """
    保存收入明细到数据库
