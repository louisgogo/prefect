...
import pandas as pd
from mypackage.utilities import add_data, connect_to_db, val_dist
...
def load_profit_data_task(date_range: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    try:
        ...
        df_profit = strip_source_metadata_columns(df_profit)
...
def process_manual_profit_task(
    ...
    try:
        ...
        df_profit_bus_hand["mo_amt"] = (
            df_profit_bus_hand["mo_amt"] * df_profit_bus_hand["rate"]
        )
...
def convert_expense_to_profit_task(df_expense_bus_all: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        print(
            f"转换费用表为利润表格式完成，共 {len(df_expense_bus_all_to_profit)} 条记录"
        )
...
def convert_revenue_to_profit_task(
    ...
    try:
        ...
        print(
            f"转换收入表为利润表格式完成，共 {len(df_revenue_bus_all_to_profit)} 条记录"
        )
...
def load_offset_data_task(date_range: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    try:
        ...
        # 处理每年的第一个月
        df_offset_calc.loc[df_offset_calc["date"].dt.month == 1, "mo_amt"] = (
            df_offset_calc["offset_num"]
        )
        ...
        # 过滤零值和空值
        df_offset = df_offset[
            (df_offset["mo_amt"] != 0) & (df_offset["mo_amt"].notna())
        ]
...
def merge_profit_data_task(
    ...
    try:
        ...
        df_profit_bus_all_to_profit = df_profit_bus_all_to_profit[
            ...
            & (df_profit_bus_all_to_profit["mo_amt"] != 0)
        ]
...
def process_shared_profit_task(date_range: pd.DatetimeIndex) -> None:
    ...
    try:
        ...
        print(
            f"正在查询利润表数据（日期范围：{date_range.min()} 到 {date_range.max()}）..."
        )
        ...
        if len(df_shared) > 0:
            df_shared_profit = (
                df_shared[df_index].groupby(df_groupby).sum().reset_index()
            )
            ...
            df_shared_profit["mo_amt"] = (
                df_shared_profit["mo_amt"] * df_shared_profit["rate"]
            )
        ...
        if len(df_shared) > 0:
            ...
            add_data("fact_bus_profit_bd", df_shared_profit_all)
            print(
                f"处理预提费用综合比例公摊利润完成，共 {len(df_shared_profit_all)} 条记录"
            )
        ...
        if len(df_shared) > 0:
            ...
            add_data("fact_bus_profit_bd", df_shared_profit_all)
            print(
                f"处理预提费用特殊比例公摊利润完成，共 {len(df_shared_profit_all)} 条记录"
            )
