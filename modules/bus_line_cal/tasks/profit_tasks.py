...
import pandas as pd
from mypackage.utilities import add_data, connect_to_db, delete_data_add_data_by_DateRange, val_dist
...
def load_profit_data_task(date_range: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    try:
        ...
        df_profit = pd.DataFrame(cur.fetchall(), columns=[desc[0] for desc in cur.description])
...
def process_manual_profit_task(
    df_profit: pd.DataFrame, df_upload_merge_all: pd.DataFrame
) -> pd.DataFrame:
    """
    处理手工分拆的利润数据
    ...
    try:
        ...
        df_profit_bus_hand = df_profit_bus_hand.astype({"rate": "float", "mo_amt": "float"})
        df_profit_bus_hand["mo_amt"] = df_profit_bus_hand["mo_amt"] * df_profit_bus_hand["rate"]
        df_profit_bus_hand = df_profit_bus_hand.drop(["id"], axis=1, errors="ignore")
...
def process_auto_profit_task(
    df_profit: pd.DataFrame, df_upload_merge_all: pd.DataFrame, df_org: pd.DataFrame
) -> pd.DataFrame:
    """
    处理自动归属的利润数据
...
def convert_expense_to_profit_task(df_expense_bus_all: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    try:
        df_expense_bus_all_to_profit = df_expense_bus_all.drop(
            ["date"], axis=1, errors="ignore"
        ).rename(columns={"exp_amt": "mo_amt", "acct_period": "date"})
...
def load_offset_data_task(date_range: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    try:
        conn, cur = connect_to_db()
        cur.execute("SELECT * FROM fact_offset WHERE subj_name NOT IN ('营业利润','净利润','利润总额')")
        df_offset = pd.DataFrame(cur.fetchall(), columns=[desc[0] for desc in cur.description])
        ...
        df_offset_calc = df_offset[
            (df_offset["date"].dt.year == year) & (df_offset["date"] <= max_date)
        ].copy()

        # 按科目、日期、唯一层级分组汇总
        df_offset_calc = (
            df_offset_calc.groupby(["subj_name", "date", "unique_lvl"])
            .agg({"offset_num": "sum"})
            .reset_index()
        )

        # 按年份分组并应用 diff()
        df_offset_calc = df_offset_calc.sort_values(["unique_lvl", "subj_name", "date"])
        df_offset_calc["mo_amt"] = df_offset_calc.groupby(["unique_lvl", "subj_name"])[
            "offset_num"
        ].diff()

        # 处理每年的第一个月
        df_offset_calc.loc[df_offset_calc["date"].dt.month == 1, "mo_amt"] = df_offset_calc[
            "offset_num"
        ]

        # 现在才筛选到目标日期范围（最终结果）
        df_offset = df_offset_calc[df_offset_calc["date"].isin(date_range)].copy()
        ...
        # 过滤零值和空值
        df_offset = df_offset[(df_offset["mo_amt"] != 0) & (df_offset["mo_amt"].notna())]
        ...
        df_offset_to_save["acct_period"] = pd.to_datetime(df_offset_to_save["acct_period"])
        df_offset_to_save["class"] = "抵销"
        # source_no 格式：'O' + index.astype(str)
        df_offset_to_save = df_offset_to_save.reset_index(drop=True)
        df_offset_to_save["source_no"] = "O" + df_offset_to_save.index.astype(str)
        # 只保留需要的列
        ...
        print(f"抵销数数据已保存到 fact_offset_by_month 表，共 {len(df_offset_to_save)} 条记录")
...
def merge_profit_data_task(
    ...
    try:
        ...
        df_profit_bus_all_to_profit["unique_lvl"] = df_profit_bus_all_to_profit[
            "unique_lvl"
        ].replace("无归属-其他调整-待摊事项", "新国都本部-公共部门-公共部门")
...
def save_profit_detail_task(df: pd.DataFrame, date_range: pd.DatetimeIndex) -> None:
    """
    保存利润明细到数据库
...
def process_shared_profit_task(date_range: pd.DatetimeIndex) -> None:
    """
    处理业务线为"无"的利润数据分摊
    ...
    try:
        ...
        # 获取预提类型为综合比例的其他
        df_profit_bus_pro_list_z = df_upload_merge_all[df_upload_merge_all["class"] == "预提-综合比例"]
        df_profit_bus_pro_list_z = df_profit_bus_pro_list_z["source_no"].tolist()

        # 获取预提类型为特殊比例的其他
        df_profit_bus_pro_list_t = df_upload_merge_all[df_upload_merge_all["class"] == "预提-特殊比例"]
        ...
        df_all = pd.DataFrame(cur.fetchall(), columns=[desc[0] for desc in cur.description])
        print(f"查询到 {len(df_all)} 条利润数据")
        df_all = df_all.rename(columns={"unique_lvl": "source_lvl", "sec_dist_lvl": "unique_lvl"})

        # 定义逆透视的行标签列
        df_index = ["date", "year", "source_no", "unique_lvl", "source_lvl", "prim_subj", "mo_amt"]
        df_groupby = df_index[:-1]

        # 计算非预提费用的，无的净利润
        df_shared = df_all[
            (df_all["bus_line"] == "无")
            & (~df_all["source_no"].isin(df_profit_bus_pro_list_z + df_profit_bus_pro_list_t))
        ]

        if len(df_shared) > 0:
            df_shared_profit = df_shared[df_index].groupby(df_groupby).sum().reset_index()
            df_shared_profit = df_shared_profit.merge(df_shared_rate, how="left", on="date")

            df_shared_profit["mo_amt"] = df_shared_profit["mo_amt"].astype(float)
            df_shared_profit["mo_amt"] = df_shared_profit["mo_amt"] * df_shared_profit["rate"]
            ...
            df_shared_profit = df_shared_profit[
                (df_shared_profit["mo_amt"] != 0) & (df_shared_profit["mo_amt"].notnull())
            ]
            ...
            if "id" in df_shared_profit_all.columns:
                df_shared_profit_all = df_shared_profit_all.drop(["id"], axis=1)
            add_data("fact_bus_profit_bd", df_shared_profit_all)
            print(f"处理非预提费用公摊利润完成，共 {len(df_shared_profit_all)} 条记录")

        # 计算预提费用的，业务线为无，适用综合比例
        df_shared = df_all[
            (df_all["bus_line"] == "无") & (df_all["source_no"].isin(df_profit_bus_pro_list_z))
        ]
        ...
        df_shared_rate_t = df_upload_merge_all[df_upload_merge_all["class"] == "预提-特殊比例"][
            ["source_no", "bus_line", "rate"]
        ]

        df_shared = df_all[
            (df_all["bus_line"] == "无") & (df_all["source_no"].isin(df_profit_bus_pro_list_t))
        ]

        if len(df_shared) > 0:
            df_shared_profit = df_shared[df_index].groupby(df_groupby).sum().reset_index()
            df_shared_profit = df_shared_profit.merge(df_shared_rate_t, how="left", on="source_no")
            ...
            df_shared_profit["source_lvl"] = df_shared_profit.apply(
                lambda x: "新国都本部-公共部门-公共部门"
                if x["source_lvl"] == "无归属-其他调整-待摊事项"
                else x["source_lvl"],
                axis=1,
            )
            df_shared_profit["category"] = "预提费用"
