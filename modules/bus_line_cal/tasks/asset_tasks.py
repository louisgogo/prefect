...
def process_receivable_task(
    df_ar: pd.DataFrame, df_bus_line: pd.DataFrame, df_org: pd.DataFrame
) -> pd.DataFrame:
    """
    处理应收数据并应用业务线比例
...
def save_receivable_detail_task(df: pd.DataFrame, date_range: pd.DatetimeIndex) -> None:
    """
    保存应收明细到数据库
...
def process_inventory_task(
    df_inv: pd.DataFrame, df_bus_line: pd.DataFrame, df_org: pd.DataFrame
) -> pd.DataFrame:
    """
    处理存货数据并应用业务线比例
...
def save_inventory_detail_task(df: pd.DataFrame, date_range: pd.DatetimeIndex) -> None:
    """
    保存存货明细到数据库
...
def process_inventory_on_way_task(
    df_inv_on: pd.DataFrame, df_bus_line: pd.DataFrame, df_org: pd.DataFrame
) -> pd.DataFrame:
    """
    处理在途存货数据并应用业务线比例
