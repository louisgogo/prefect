...
def load_expense_data_task(date_range: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    try:
        ...
        cur.execute(
            "SELECT * FROM fact_expense WHERE unique_lvl NOT LIKE %s AND acct_period >= %s AND acct_period <= %s",
            ("%无归属%", date_range.min(), date_range.max()),
        )
        df_expense = pd.DataFrame(cur.fetchall(), columns=[desc[0] for desc in cur.description])
        df_expense = strip_source_metadata_columns(df_expense)
...
def _normalize_expense_split_rates(
    df_upload_merge_all: pd.DataFrame, df_expense: pd.DataFrame
) -> pd.DataFrame:
    """
    归一化费用拆分比例。

    cal_person_weight() 对行政/人力中心费用会按多个组织层级（unique_lvl）生成比例，
    每个 source_no 可能对应多条比例记录。本函数按 (source_no, bus_line) 聚合后，
    再对每个 source_no 的 rate 做归一化，确保单条费用金额不会被重复放大。
    """
    df = df_upload_merge_all[df_upload_merge_all["class"] == "费用"].copy()
    if df.empty:
        return df

    # 只保留当前处理期间的比例数据，避免跨月数据干扰
    df["acct_period"] = pd.to_datetime(df["acct_period"])
    df_expense_periods = pd.to_datetime(df_expense["acct_period"]).unique()
    df = df[df["acct_period"].isin(df_expense_periods)]

    if df.empty:
        return df

    # 按 (source_no, bus_line) 聚合比例
    df = df.groupby(["source_no", "bus_line"], as_index=False).agg(
        {"rate": "sum", "unique_lvl": "first", "category": "first"}
    )
    df = df[df["rate"].notna() & (df["rate"] != 0)]

    # 归一化：保证每个 source_no 的 rate 之和严格等于 1
    rate_sums = df.groupby("source_no")["rate"].transform("sum")
    df["rate"] = df["rate"] / rate_sums
...
def process_gap_expense_task(
    df_expense: pd.DataFrame, df_upload_merge_all: pd.DataFrame
) -> pd.DataFrame:
    """
    处理比率不完整的费用（公摊费用）
    ...
    """
    try:
        df_expense_bus = _normalize_expense_split_rates(df_upload_merge_all, df_expense)

        # 找出比率不完整的费用
        df_expense_bus_gap_rate = (
            df_expense_bus.groupby(["source_no"]).agg({"rate": "sum"}).reset_index()
        )
        df_expense_bus_gap_rate = df_expense_bus_gap_rate[df_expense_bus_gap_rate["rate"] < 1]
        df_expense_bus_gap_rate["rate"] = 1 - df_expense_bus_gap_rate["rate"].astype("float")
        df_expense_bus_gap_list = df_expense_bus_gap_rate["source_no"].tolist()

        # 处理公摊费用
        df_expense_bus_gap = df_expense[
            df_expense["source_no"].isin(df_expense_bus_gap_list)
        ].merge(df_expense_bus_gap_rate[["source_no", "rate"]], on=["source_no"], how="left")
        df_expense_bus_gap = df_expense_bus_gap.astype({"rate": "float", "exp_amt": "float"})
        df_expense_bus_gap["exp_amt"] = df_expense_bus_gap["exp_amt"] * df_expense_bus_gap["rate"]
...
def process_manual_expense_task(
    ...
    try:
        ...
        df_expense_bus_hand = df_expense[df_expense["source_no"].isin(df_expense_bus_list)].merge(
            df_expense_bus[["source_no", "bus_line", "unique_lvl", "category", "rate"]],
            on=["source_no"],
            how="left",
        )
        df_expense_bus_hand = df_expense_bus_hand.astype({"rate": "float", "exp_amt": "float"})
        df_expense_bus_hand = df_expense_bus_hand.rename(
            columns={"unique_lvl_x": "source_lvl", "unique_lvl_y": "unique_lvl"}
        )
        df_expense_bus_hand["exp_amt"] = (
            df_expense_bus_hand["exp_amt"] * df_expense_bus_hand["rate"]
        )
...
def process_auto_expense_task(
    df_expense: pd.DataFrame, df_upload_merge_all: pd.DataFrame, df_org: pd.DataFrame
) -> pd.DataFrame:
    """
    处理自动归属的费用数据
...
def merge_expense_data_task(
    ...
    try:
        ...
        df_expense_bus_all = df_expense_bus_all.drop(["id"], axis=1, errors="ignore")
...
def apply_shared_rate_to_expense_task(
    df_expense_bus_all: pd.DataFrame, date_range: pd.DatetimeIndex
) -> pd.DataFrame:
    """
...
def save_expense_detail_task(df: pd.DataFrame, date_range: pd.DatetimeIndex) -> None:
    """
    保存费用明细到数据库
