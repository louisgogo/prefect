...
def load_human_cost_for_shared_rate_task(date_range: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    try:
        ...
        # 计算每个组织在各业务线的费用比例
        # 按日期和组织汇总总费用
        df_total = df.groupby(["date", "unique_lvl"])["total_expense"].sum().reset_index()
        df_total = df_total.rename(columns={"total_expense": "total_org_expense"})
...
def calculate_personnel_allocation_task(
    df_personnel: pd.DataFrame, df_human_cost: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        df_merged = df_personnel.merge(df_human_cost, on=["date", "unique_lvl"], how="left")

        # 计算分配到各业务线的人数
        df_merged["personnel_count"] = df_merged["person_count"] * df_merged["expense_ratio"]
        df_merged = df_merged[
            df_merged["personnel_count"].notna() & (df_merged["personnel_count"] != 0)
        ]

        # 按 date 和 bus_line 汇总
        df_result = df_merged.groupby(["date", "bus_line"])["personnel_count"].sum().reset_index()
...
def calculate_comprehensive_rate_task(
    df_profit: pd.DataFrame, df_personnel_allocation: pd.DataFrame
) -> pd.DataFrame:
    """
    计算四个指标的加权平均（等权重25%）
    ...
    try:
        ...
        df_revenue_sum = df_revenue.groupby(["date", "bus_line"])["amt"].sum().reset_index()
        df_revenue_sum = df_revenue_sum.rename(columns={"amt": "revenue"})

        df_gross_profit_sum = (
            df_gross_profit.groupby(["date", "bus_line"])["amt"].sum().reset_index()
        )
        df_gross_profit_sum = df_gross_profit_sum.rename(columns={"amt": "gross_profit"})

        df_net_profit_sum = df_net_profit.groupby(["date", "bus_line"])["amt"].sum().reset_index()
        df_net_profit_sum = df_net_profit_sum.rename(columns={"amt": "net_profit"})

        # 合并所有指标
        df_all = (
            df_revenue_sum.merge(df_gross_profit_sum, on=["date", "bus_line"], how="outer")
            .merge(df_net_profit_sum, on=["date", "bus_line"], how="outer")
            .merge(df_personnel_allocation, on=["date", "bus_line"], how="outer")
        )
        ...
        numeric_cols = ["revenue", "gross_profit", "net_profit", "personnel_count"]
        for col in numeric_cols:
            if col in df_all.columns:
                df_all[col] = pd.to_numeric(df_all[col], errors="coerce").astype(float)

        # 对所有金额指标应用绝对值，确保都是正数
        amount_cols = ["revenue", "gross_profit", "net_profit", "personnel_count"]
        ...
        # 确保总和列也是 float 类型
        total_cols = ["total_revenue", "total_gross_profit", "total_net_profit", "total_personnel"]
        ...
        df_all["revenue_ratio"] = df_all.apply(
            lambda x: x["revenue"] / x["total_revenue"] if x["total_revenue"] != 0 else 0.0, axis=1
        )
        df_all["gross_profit_ratio"] = df_all.apply(
            lambda x: x["gross_profit"] / x["total_gross_profit"]
            if x["total_gross_profit"] != 0
            else 0.0,
            axis=1,
        )
        df_all["net_profit_ratio"] = df_all.apply(
            lambda x: x["net_profit"] / x["total_net_profit"]
            if x["total_net_profit"] != 0
            else 0.0,
            axis=1,
        )
        df_all["personnel_ratio"] = df_all.apply(
            lambda x: x["personnel_count"] / x["total_personnel"]
            if x["total_personnel"] != 0
            else 0.0,
            axis=1,
        )

        # 确保比例列也是 float 类型
        ratio_cols = ["revenue_ratio", "gross_profit_ratio", "net_profit_ratio", "personnel_ratio"]
        ...
        for date in sorted(df_all["date"].unique()):
            ...
            print("-" * 80)
            print(f"{'业务线':<20} {'收入比例':<12} {'毛利润比例':<12} {'净利润比例':<12} {'人数比例':<12} {'综合比例':<12}")
            print("-" * 80)
            ...
            print("-" * 80)
            print(
                f"总计 - 收入: {df_date['revenue'].sum():,.2f}, "
        ...
        df_result = df_result[df_result["rate"].notna() & (df_result["rate"] != 0)]
...
def save_shared_rate_task(df_shared_rate: pd.DataFrame, date_range: pd.DatetimeIndex) -> None:
    """
    ...
    try:
        ...
        # 只保留日期范围内的数据
        df_shared_rate = df_shared_rate[df_shared_rate["date"].isin(date_range)]
