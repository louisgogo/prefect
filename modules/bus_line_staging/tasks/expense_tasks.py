...
def run_expense_split_to_staging_task(date_range, batch_id):
    ...
    if df_labor.empty:
        raise ValueError(
            "fact_bus_wage_rate 表中没有数据，无法计算后台人力及分摊费用（含房租）的业务线比例。"
            "请先更新 fact_bus_wage_rate 表后再运行本流程。"
        )
    ...
    if df_labor.empty:
        raise ValueError(
            f"fact_bus_wage_rate 表中没有 {date_range[0]} 到 {date_range[-1]} 的数据，"
            "无法计算后台人力及分摊费用（含房租）的业务线比例。"
