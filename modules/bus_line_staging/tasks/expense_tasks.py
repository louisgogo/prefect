...
def run_expense_split_to_staging_task(date_range):
    ...
    # 费用科目定义
    expense_categorys_1 = ['FYXM0012', 'FYXM0014', 'FYXM0016', 'FYXM0024', 'FYXM0025', 'FYXM0026', 'FYXM0027', 'FYXM0028', 'FYXM0029', 'FYXM0031', 'FYXM0072', 'FYXM0073']
    ...
    # 获取原始费用数据
    cur.execute(f"SELECT * FROM fact_expense WHERE (exp_item_code in({expense_categorys_text_1}) AND unique_lvl like '%行政中心%') OR (exp_item_code in({expense_categorys_text_2}) AND unique_lvl like '%人力资源中心%')")
    ...
    df_org_all = pd.DataFrame(cur.fetchall(), columns=['unique_lvl', 'prim_org', 'sec_org', 'third_org', 'short_name', 'category'])
    
    # 计算人数比例
    cur.execute('''
        SELECT * FROM fact_personnel 
        WHERE (unique_lvl NOT LIKE '%分公司%' OR third_org in ('增值业务产品部','分公司运营支持部','通道合作部','银行合作部'))
    ...
    df_template = df_template.reset_index()[['来源编号', '唯一层级','单据编号','报销人', '摘要', '会计期间', '费用性质', '费用大类','核算项目-费控', '研发项目', '项目编码', '费用金额','数据来源','分摊业务线']]
    df_template[['一级组织', '二级组织', '三级组织']] = df_template['唯一层级'].astype(str).str.split('-', n=2, expand=True)
    ...
    df_template = df_template[['来源编号', '唯一层级', '一级组织', '二级组织','三级组织','单据编号','报销人', '摘要', '会计期间', '费用性质', '费用大类', '核算项目-费控','研发项目', '项目编码', '费用金额', '年份','数据来源','分摊业务线']]
    ...
    insert_to_staging_table(df=df_template, df_org=df_middle, groups=groups_middle, date_range=date_range, date_column=date_column, table_name=table_name, bus_lines=bus_lines, is_split_others=False, is_by_df=False)

    print("开始处理 后台自身数据拆分（不含人力等）...")
    df_backend_org = df_org_all[df_org_all.category.isin(['后台'])]
    df_backend = df_template[(df_template['费用性质'] != '人力费用') & ~(df_template['来源编号'].isin(id_exclude))].copy()
    insert_to_staging_table(df=df_backend, df_org=df_backend_org, groups=groups_backend, date_range=date_range, date_column=date_column, table_name=table_name, bus_lines=bus_lines, is_split_others=True, is_by_df=False)

    print("开始处理 后台自身数据拆分（含人力和分摊）...")
    ...
    if not df_labor.empty:
        ...
        df_1 = df_template[(df_template['费用性质'] == '人力费用') | (df_template['来源编号'].isin(id_exclude))].melt(
          id_vars=['来源编号', '唯一层级', '一级组织', '二级组织', '三级组织','单据编号', '报销人', '摘要', '会计期间', '费用性质', '费用大类', '核算项目-费控', '研发项目', '项目编码', '费用金额', '年份', 'sec_dist_lvl', '分摊业务线'],
          var_name='业务线', value_name='比例').drop('比例', axis=1)
          
        df_a = pd.merge(df_1, df_labor, how='left', on=['唯一层级','业务线','会计期间'])
        if not df_a.empty:
            idx_cols = ['来源编号', '唯一层级', '一级组织', '二级组织', '三级组织', '单据编号',
                        '报销人', '摘要', '会计期间', '费用性质', '费用大类', '核算项目-费控',
                        '研发项目', '项目编码', '费用金额', '年份', 'sec_dist_lvl', '分摊业务线']
            # 用 pivot_table 代替 pivot，避免同一 index 组合多行导致的 ValueError
            df_labor_backend = df_a.pivot_table(
                columns='业务线', values='比例',
                index=idx_cols,
                aggfunc='first'
            ).reset_index()
            ...
            for col in bus_lines:
                df_labor_backend[col] = np.where(df_labor_backend['分摊业务线'] == col, 1, df_labor_backend[col])
            insert_to_staging_table(df=df_labor_backend, df_org=df_backend_org, groups=[], date_range=date_range, date_column=date_column, table_name=table_name, bus_lines=bus_lines, is_split_others=True, is_by_df=False)

    print("开始处理 前台中需要单独拆分的业务线...")
    df_frontend = df_org_all[df_org_all.category.isin(['前台'])]
    insert_to_staging_table(df=df_template, df_org=df_frontend, groups=groups_frontend, date_range=date_range, date_column=date_column, table_name=table_name, bus_lines=bus_lines, is_split_others=False, is_by_df=True)
