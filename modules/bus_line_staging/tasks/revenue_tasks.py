...
def run_revenue_other_split_task(date_range):
    ...
    try:
        ...
        # 2. 其他项目数据
        print("正在获取其他项目数据...")
        cur.execute("SELECT * FROM fact_profit_bd")
        df_profit = pd.DataFrame(cur.fetchall(), columns=[reverse_combined_column_mapping.get(desc[0], desc[0]) for desc in cur.description])
        ...
        if not df_revenue.empty:
            insert_to_staging_table(df=df_revenue, df_org=df_path, groups=groups_frontend, date_range=date_range, date_column='会计期间', table_name='staging_bus_revenue', bus_lines=bus_lines, is_split_others=False, is_by_df=True)
        if not df_profit.empty:
            insert_to_staging_table(df=df_profit, df_org=df_path, groups=groups_frontend, date_range=date_range, date_column='日期', table_name='staging_bus_profit_bd', bus_lines=bus_lines, is_split_others=False, is_by_df=True)
