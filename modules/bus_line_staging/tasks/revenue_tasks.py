...
def run_revenue_other_split_task(date_range, batch_id, modules: Sequence[str] | None = None):
    print("开始执行: 2-特定部门的收入、其他数据拆分(入库中间表)")

    selected = {"revenue", "profit_other"} if modules is None else set(modules)
    unknown = selected.difference({"revenue", "profit_other"})
    if unknown:
        raise ValueError(f"收入/其他拆分收到不支持的模块: {', '.join(sorted(unknown))}")
    ...
    try:
        ...
        if "revenue" in selected:
            ...
            df_revenue = pd.DataFrame(
                cur.fetchall(),
                columns=[
                    reverse_combined_column_mapping.get(desc[0], desc[0])
        ...
        if "profit_other" in selected:
            ...
            if not df_profit.empty:
                ...
                df_profit[["一级组织", "二级组织", "三级组织"]] = df_profit["唯一层级"].str.split(
                    "-", n=2, expand=True
                )
                df_profit = df_profit[
                    [
